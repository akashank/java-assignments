...
public class parked_Owner_List {

	TreeMap<Integer, parked_Owner_Details> parkings;

	public parked_Owner_List() {
		this.parkings = new TreeMap<>();
	}

	public int add_new_car(parked_Owner_Details obj) {
		int i = 0;
		if (parkings.size() == 0) {
			parkings.put(1, obj);
			return 1;
	...
	public void removeCar(int x) {
		if (parkings.containsKey(x)) {
			parked_Owner_Details rem = (parked_Owner_Details) (parkings.remove(x));
	...
	public void get_parked_location(int token) {
		...
		if (parkings.containsKey(token)) {
			if (token >= 1 && token <= 80)
				...
				floor = 3;
			section = (token % 80) + 1;
			section = (section >= 1 && section <= 20) ? 1
					: (section > 20 && section <= 40) ? 2 : (section >= 41 && section <= 60) ? 3 : 4;
			slot = (token % 20 == 0) ? 20 : token % 20;
			System.out.println("The token " + token + " is present at " + floor + " floor, " + section + " section, "
					+ slot + " slot");
